...
def plot_heuristic_correlation(F1, F2, F3, hm, num_dm_dec, dataset_fold, sub_fold, save_figs=False, show_figs=True):
    """
    Plots the correlation between BALD and US scores over time.
    """
    f1, f2, f3 = F1[0], F2[0], F3[0]
    ...
    for i in range(hm):
        ...
        if os.path.exists(path):
            corrs = np.load(path)
            # Ensure length matches num_dm_dec
            if len(corrs) < num_dm_dec:
                 # Pad with nan
                 padded = np.full(num_dm_dec, np.nan)
                 padded[:len(corrs)] = corrs
                 all_corrs.append(padded)
            else:
                all_corrs.append(corrs[:num_dm_dec])
    
    if not all_corrs:
        print(f"No heuristic data found for {sub_fold}")
        return
    ...
    x = np.arange(1, len(mean_corr) + 1)
    
    plt.figure(figsize=(10, 6))
    plt.plot(x, mean_corr, label="Correlation (BALD vs US)", color="purple", linewidth=2)
    plt.fill_between(x, mean_corr - std_corr, mean_corr + std_corr, color="purple", alpha=0.15)
    ...
    # Shade background to indicate suggestion regions
    plt.axhspan(0, ymax, facecolor='green', alpha=0.05, label="Suggest BALD (Corr > 0)")
    plt.axhspan(ymin, 0, facecolor='orange', alpha=0.05, label="Suggest US (Corr <= 0)")
    
    plt.xlabel("Number of DM Preferences")
    plt.ylabel("Pearson Correlation")
    plt.title(f"Heuristic Check: BALD vs US Correlation ({sub_fold})")
...
def plot_heuristic_stats(F1, F2, F3, hm, num_dm_dec, dataset_fold, sub_fold, save_figs=False, show_figs=False):
    """
    ...
    for i in range(hm):
        table_dir = os.path.join(config_dir, f"table_{i}")
        file_path = os.path.join(table_dir, "active_heuristic_stats.npy")
        
        if os.path.exists(file_path):
            # Shape: (num_steps, 4) -> [mean_us, std_us, mean_bald, std_bald]
            data = np.load(file_path)
            # Handle potential shape mismatch or empty
            if data.ndim == 2:
                if data.shape[1] >= 6:
                    # New Format: [mean_us, std_us, max_us, mean_bald, std_bald, max_bald]
                    # We want: [mean_us(0), std_us(1), mean_bald(3), std_bald(4)]
                    subset = data[:, [0, 1, 3, 4]]
                    all_stats.append(subset)
                elif data.shape[1] == 4:
                    # Old Format: [mean_us, std_us, mean_bald, std_bald]
                    all_stats.append(data)
    
    if not all_stats:
        print(f"No heuristic stats found for {sub_fold}")
        return

    # Stack: (num_runs, num_steps, 4)
    # If run lengths differ, truncate to min length
    min_len = min(len(d) for d in all_stats)
    all_stats = np.array([d[:min_len] for d in all_stats])
    
    # Average across runs (HMs)
    # mean_stats shape: (num_steps, 4)
    mean_stats = np.nanmean(all_stats, axis=0) 
    
    # unpack
    us_means = mean_stats[:, 0]
    us_stds = mean_stats[:, 1]
    bald_means = mean_stats[:, 2]
    bald_stds = mean_stats[:, 3]
    
    steps = np.arange(1, len(us_means) + 1)
    
    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    
    # 1. US Plot
    axes[0].plot(steps, us_means, color='blue', linewidth=2, label='Mean Entropy')
    axes[0].fill_between(steps, us_means - us_stds, us_means + us_stds, color='blue', alpha=0.2, label='Std Dev')
    axes[0].set_xlabel("Number of DM Preferences")
    axes[0].set_ylabel("Entropy (US)")
    axes[0].set_title(f"US Scores Evolution ({sub_fold})")
    axes[0].grid(True, linestyle=':', alpha=0.6)
    axes[0].legend()
    
    # 2. BALD Plot
    axes[1].plot(steps, bald_means, color='red', linewidth=2, label='Mean Mutual Info')
    axes[1].fill_between(steps, bald_means - bald_stds, bald_means + bald_stds, color='red', alpha=0.2, label='Std Dev')
    axes[1].set_xlabel("Number of DM Preferences")
    axes[1].set_ylabel("Mutual Information (BALD)")
    axes[1].set_title(f"BALD Scores Evolution ({sub_fold})")
...
def plot_heuristic_pearson_pvalues(F1, F2, F3, hm, num_dm_dec, dataset_fold, sub_fold, save_figs=False, show_figs=False, threshold=0.01):
    """
    ...
    for i in range(hm):
        table_dir = os.path.join(config_dir, f"table_{i}")
        file_path = os.path.join(table_dir, "active_pearson_pvalues.npy")
        
        if os.path.exists(file_path):
            data = np.load(file_path)
            if data.ndim == 1:
                all_pvals.append(data)
    
    if not all_pvals:
        print(f"No Pearson p-value stats found for {sub_fold}")
        return

    min_len = min(len(d) for d in all_pvals)
    all_pvals = np.array([d[:min_len] for d in all_pvals])
    
    mean_pvals = np.nanmean(all_pvals, axis=0)
    std_pvals = np.nanstd(all_pvals, axis=0)
    
    steps = np.arange(1, len(mean_pvals) + 1)
    
    plt.figure(figsize=(8, 6))
    plt.plot(steps, mean_pvals, label="Mean Pearson p-value", color='purple', linewidth=2)
    plt.fill_between(steps, mean_pvals - std_pvals, mean_pvals + std_pvals, color='purple', alpha=0.15)
    
    plt.axhline(threshold, color='green', linestyle='--', label=f'Threshold ({threshold})')
    plt.yscale('log')
    
    # Text Annotations
    plt.text(steps[0], threshold * 1.2, "Else: BALD", verticalalignment='bottom', color='gray')
    plt.text(steps[0], threshold * 0.8, f"Left-Tail p < {threshold}: US", verticalalignment='top', color='gray')
    
    plt.xlabel("Number of DM Preferences")
    plt.ylabel("p-value (Pearson) [Log Scale]")
    plt.title(f"Heuristic: Pearson Correlation Significance ({sub_fold})")
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.legend()
    plt.tight_layout()
